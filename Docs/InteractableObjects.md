...
| D | `InteractableObjectD` | Trigger | 鬼魂：可反复对话，每关通常一个 |

同一关卡若有多套机制，用基类上的 **`mechanismId`** 绑定：同一机制的 A / B / C 填相同 id，彼此不可交叉。

```
玩家按 E
  ├─ 对话框打开中 → 关闭对话框（不丢弃/不交互）
  ├─ 附近有可对话的 D → 打开对话框
...
- 继承基类，`Mode = Trigger`，可反复互动。
- 每关建议只放一个；多放会打 Warning。
- 按 E 显示自身子物体 `PopupPanel`（挂 `DialoguePopup`）：
  - 运行时挂到 `UIManager.UIRoot`（Screen Space Overlay）
...
1. 所有可放置物体（地形 / Interactable）继承 `LevelObject`，必须设置 `levelLayer`：`A` / `B` / `Shared`。
2. `Shared` 不随世界切换；`A`/`B` 随 Shift 显隐。编辑器放置时也可覆盖层级。
3. 每套机制选一个唯一 `mechanismId`（如 `door_1`、`puzzle_red`）。
4. 该机制下的 A / B / C 都填这个 id。
5. B 若要顺序触发：同组 `sequenceGroupId` + 递增 `sequenceIndex`，按需调 `resetDelaySeconds`。
6. D 填 `dialogueText`；每关通常一个。
7. 物体需有 Collider2D（建议 Trigger）；玩家需有交互范围 Trigger + `PlayerInteractor`。

## 相关脚本

- `Assets/Scripts/Gameplay/LevelObject.cs` — 关卡物体基类（含 `levelLayer`）
- `Assets/Scripts/Gameplay/TerrainTile.cs` — 地形
- `Assets/Scripts/Gameplay/InteractableObject.cs` — 交互基类 / A
- `Assets/Scripts/Gameplay/InteractableObjectB.cs` — B
- `Assets/Scripts/Gameplay/InteractableObjectC.cs` — C
- `Assets/Scripts/Gameplay/InteractableObjectD.cs` — D（鬼魂）
- `Assets/Scripts/UI/DialoguePopup.cs` — 对话弹窗
